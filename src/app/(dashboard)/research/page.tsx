...
/**
 * Research Analyst page — Plan 1 Walking Skeleton.
 *
 * Wires the existing `useChat` transport pattern (`/api/chat` analog) to
 * the new `/api/research` route. The response is a streamed UI-message
 * stream emitting `data-activity` events + a markdown `text-delta` body
 * (currently a stub from `runResearchStream`).
 *
 * companyId is sourced from the `?companyId=<uuid>` query param per
 * UI-SPEC Plan 1 (a real company selector is out of scope for the
 * Walking Skeleton; CONTEXT.md OQ-3).
 */
export default function ResearchPage() {
  ...
  const transport = useMemo(
    () =>
      new DefaultChatTransport<UIMessage>({
        api: "/api/research",
        // Override the default body shape. `/api/research` expects
        // { query, companyId } per researchRequestSchema — NOT the
        // `messages` array that DefaultChatTransport sends by default.
  ...
  // Derive activity + report text from the latest assistant message's parts.
  ...
  // No company in URL — render a banner instead of the prompt input.
  if (!companyId) {
    return (
      <main className="mx-auto flex w-full max-w-6xl flex-col gap-6 bg-zinc-950 px-6 py-8 font-sans text-zinc-200">
        ...
        <div className="rounded-[10px] border border-zinc-800 bg-zinc-900/60 px-5 py-4">
          <p className="text-sm text-zinc-300">
            You don&apos;t have an active company. Visit{" "}
            <Link
              href="/dashboard"
            ...
            to pick one, then return to{" "}
            <code className="text-zinc-200">/research?companyId=&lt;uuid&gt;</code>.
          </p>
        </div>
      </main>
    );
  ...
  return (
    <main className="mx-auto flex w-full max-w-6xl flex-col gap-6 bg-zinc-950 px-6 py-8 font-sans text-zinc-200">
      ...
      {/* Two-column grid: Activity / Report */}
